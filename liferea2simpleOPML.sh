...
# Suppression des balises "title","description","id","sortColumn","htmlUrl","updateInterval","collapsed", "expanded"
sed -Ei 's/ title="[^"]*"| description="[^"]*"| id="[^"]*"| sortColumn="[^"]*"| htmlUrl="[^"]*"| updateInterval="[^"]*"| collapsed="[^"]*"| expanded="[^"]*"//g' $_feedlist_simple


# Suppression des outlines "Non lus" et "Importants"
#sed -i '/type="rule"/{N;s/\n.*//;};/type="vfolder"/d;/type="rule"/d' $_feedlist_simple
# Suppression des balises "title","description","type","id","sortColumn","updateInterval","collapsed" et "expanded"
#sed -Ei 's/ title="[^"]*"| description="[^"]*"| type="[^"]*"| id="[^"]*"| sortColumn="[^"]*"| updateInterval="[^"]*"| collapsed="[^"]*"| expanded="[^"]*"//g' $_feedlist_simple

# Remplacement des caractères hexadecimaux HTML par des caractères normaux
sed -i 's/&#xE2;/â/g;s/&#xE0;/à/g;s/&#xE9;/é/g;s/&#xEA;/ê/g;s/&#xE8;/è/g;s/&#xEB;/ë/g;s/&#xEE;/î/g;s/&#xCF;/ï/g;s/&#xD4;/ô/g;s/&#x9C;/œ/g;s/&#xDB;/û/g;s/&#xF9;/ù/g;s/&#xFC;/ü/g;s/&#xE7;/ç/g;s/&#xC2;/Â/g;s/&#xC0;/À/g;s/&#xC9;/É/g;s/&#xCA;/Ê/g;s/&#xC8;/È/g;s/&#xCB;/Ë/g;s/&#xCE;/Î/g;s/&#xCF;/Ï/g;s/&#xD4;/Ô/g;s/&#x8C;/Œ/g;s/&#xDB;/Û/g;s/&#xD9;/Ù/g;s/&#xDC;/Ü/g;s/&#xC7;/Ç/g' $_feedlist_simple
